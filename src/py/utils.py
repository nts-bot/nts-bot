...
def logger(
    ...
    _ = os.path.join(src, ".log")
    if not os.path.exists(_):
        os.makedirs(_)

    name = f"{dt.datetime.now().strftime('%Y%m%d')}.log"
    ...
    fileHandler = logging.FileHandler(os.path.join(_, name))
...
def rnw_json(
    ...
    if store is None:
        ...
        try:
            ...
            logging.warning(f"JSON Corrupted: {filename}.json")
