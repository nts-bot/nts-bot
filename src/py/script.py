...
# .2 Local
from . import src
from .web import connection as conn
from .utils import rnw_json, timeout, process_process, thread_process
from .rating import test
...
class nts:
    def __init__(self):
        self.showlist = [i.split(".")[0] for i in os.listdir(join(src,"tracklist"))]
        self.pid = rnw_json(join(src, "pid"))
    ...
    def review(self, show):
        ...
        try:
            rday = dt.datetime.fromtimestamp(
                os.path.getmtime(join(src,"spotify",f"{show}.json"))
            ).date()
        except:
            return True
        if rday in day:
    ...
    def _reset(self, show):
        logging.info(".RESET.")
        rnw_json(join(src, "spotify_search_results", show), dict())
        rnw_json(join(src, "spotify", show), dict())
    ...
    def bite_img(self, show):
        im = [i for i in os.listdir(join(src,"jpeg")) if i.split(".")[0] == show][0]
        img = cv2.imread(join(src,"jpeg",im))
    ...
    def _pid(self, show):
        ...
        try:
            ...
            rnw_json(join(src, "pid"), self.pid)
            return ref["id"]
    ...
    def searchloop(self, show, jsonlist, kind="search", episodelist=[]):
        ...
        for jsondir in jsonlist:
            locals()[jsondir] = rnw_json(join(src, jsondir, show))
        ...
        for episode in episodelist:
            ...
            if episode not in eval(jsonlist[1]):
                ...
                rnw_json(join(src, jsonlist[1], show), eval(jsonlist[1]))
    ...
    def spotifyplaylist(
        ...
        pid = self._pid(show)
        meta = rnw_json(join(src, "meta", show))

        def _date_parse(
            date: str,
        ) -> str:
            # NOTE: 21-09-2025 they changed the date conventions
            # @ from [YY.mm.dd] to [dd bbb YYYY]
            if "." in date:
                datetime = dt.datetime.strptime(date, "%d.%m.%y")
            else:
                datetime = dt.datetime.strptime(date, "%d %b %Y")
            # return f"{split[2]}.{split[1]}.{split[0]}"
            return datetime.strftime("%y.%m.%d")

        # identify date order
        sortmeta = sorted(
            [_date_parse(value["date"]), key]
            for (key, value) in meta.items()
            if isinstance(value, dict)
        )

        """ LOAD DATA """
        reloaded = rnw_json(join(src, "reloaded"))
        uploaded = rnw_json(join(src, "uploaded", show))
        showlist = rnw_json(join(src, "tracklist", show))
        rate = rnw_json(join(src, "spotify", show))
        ...
        if not uploaded:
            ...
            uploaded = dict()
        elif show not in reloaded:
            logging.error(f".reload.{show}")
            uploaded = dict()
            reset = True
        ...
        while f:
            firstep = dt.datetime.strptime(sortmeta[ff][0], "%y.%m.%d").strftime(
                "%d.%m.%y"
            )
            if firstep != "00.00.00":
                f = False
                ...
                ff += 1
        lastep = dt.datetime.strptime(sortmeta[-1][0], "%y.%m.%d").strftime("%d.%m.%y")
        ...
        rnw_json(join(src, "spotify", show), rate)

        """ RESET CONDITION """
        if reset:
            logging.warning("RESET")
            current = connection.sp.user_playlist_tracks(
                connection.user,
                pid,
            )
        ...
        syn = f"{mis+len(set(pup))-len(set(tid))} tracks missing".strip()
        ...
        rnw_json(join(src, "uploaded", show), uploaded)

        """ NOTE RELOADED POST 11/2025 """
        reloaded[show] = 1
        rnw_json(join(src, "reloaded"), reloaded)
                ...
                for i in extent[::-1]:
                    if i in self.pid:
                        ...
                        try:
                            ...
                            logging.warning(f"FAILED {playlist_id} : {i}")
                    else:
    ...
    def follow(
        ...
        creds = rnw_json(join(src, f"{kind}dentials"))
        usrcall = round(len(self.showlist) / 200 + 0.4999)
        ranger = [i for i in range(1, usrcall + 1)]
    ...
    def subsubprivatise(
        ...
        pop: dict = connection.sp.playlist(
            pid,
            fields="followers,public",
        )

        # follow
        connection.sp.current_user_follow_playlist(pid)

        # make public if not already
        public = pop["public"]
        if public == True:
            logging.error(f"is public: {pid}")
        else:
            logging.error(f"mk public: {pid}")
            connection.sp.playlist_change_details(
                pid,
                public=True,
            )

        # get follower count
        fdict[pid] = pop["followers"]["total"]
    ...
    def subprivatise(
        ...
        while True:
            try:
                return self.subsubprivatise(pid, fdict)
    ...
    def publicise(
        ...
        inv_map = {v: k for k, v in self.pid.items()}

        # read followers dict
        followers = rnw_json(join(src, "followers"))
        logging.error(f"playlists: {values}")

        if public:
            # for pid in tqdm(values, "public"):
            #     self.subprivatise(pid, followers)
            ...
            # store followers dict
            rnw_json(join(src, "followers"), followers)

        # fetch 200 most followed
        most = {
            k: v
            for k, v in sorted(
                followers.items(),
        ...
        for i in tqdm(pids, "unfollow"):
            logging.info(f"{inv_map[i]}: {followers[i]}")
            connection.sp.current_user_unfollow_playlist(i)
        for i in tqdm(pids, "refollow"):
            logging.info(f"{inv_map[i]}: {followers[i]}")
            connection.sp.current_user_follow_playlist(i)
    ...
    def counter_opener(
        ...
        store = []
        eps: list[dict] = list(rnw_json(join(src, "tracklist", show)).values())
        for ep in eps:
            store += self.counter_filter(ep)
        return {show: store}
    ...
    def subcounter(
        ...
        tc = dict()
        ac = dict()
        # run through dictionary of shows
        for track_id in shows:
            for tid in shows[track_id]:
                artist = tid["artist"]
                track = tid["track"]

                # better capture
                name = re.sub("[\(\[].*?[\)\]]", "", track).strip().title()
                name = f"{name} - {artist}"
    ...
    def counter(self):
        ...
        # store
        with open(join(src,"most_played_tracks.txt"), "w", encoding="utf-8") as f:
            f.write(self.cleaners_from_venus(track_d, 500))
        with open(join(src,"most_played_artists.txt"), "w", encoding="utf-8") as f:
            f.write(self.cleaners_from_venus(artist_d, 200))

    @staticmethod
    def cleaners_from_venus(
        dic,
        amount,
    ):
        c = dict(
    ...
    def parallel(
        ...
        r = range(len(ranger))
        anem = "".join(random.choice(string.ascii_letters) for _ in range(10))
    ...
    def backup(
        ...
        for i in ["pid"]:
            file = rnw_json(join(src, i))
            rnw_json(join(src, "backup", i), file)
